...
class Node:
    ...
    def get_health_utility(self):
        raise NotImplementedError("This is an abstract method and need to be implemented in derived classes.")
...
class ChanceNode(Node):
    ...
    def get_health_utility(self):
        ...
        i = 0
        for node in self.futureNodes:
            exp_utility += self.probs[i]*node.get_health_utility()
            i += 1
        return exp_utility
...
class DecisionNode(Node):
    ...
    def get_health_utility(self):
        output = dict()
        for node in self.futureNode:
            output[node.name] = node.get_health_utility()
